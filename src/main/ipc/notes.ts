...
export interface CreateNoteInput {
  scope_type: 'verse' | 'verse_range' | 'chapter' | 'book'
  book_id: number
...
export function registerNotesHandlers(): void {
  ipcMain.handle('notes:getForScope', (_event, scope: NoteScope) => {
    ...
    if (scope.type === 'verse') {
      return db
        .prepare(
          `SELECT * FROM notes WHERE book_id = ? AND chapter_num = ? AND (
            (scope_type = 'verse' AND verse_start = ?) OR
            (scope_type = 'verse_range' AND verse_start <= ? AND verse_end >= ?) OR
            scope_type = 'chapter' OR scope_type = 'book'
          ) ORDER BY updated_at DESC`
        )
        .all(scope.bookId, scope.chapterNum, scope.verseNum, scope.verseNum, scope.verseNum)
    }
    return []
  })
  ...
  ipcMain.handle('notes:create', (_event, input: CreateNoteInput) => {
    ...
    const stmt = db.prepare(`
      INSERT INTO notes (scope_type, book_id, chapter_num, verse_start, verse_end, title, content)
      VALUES (@scope_type, @book_id, @chapter_num, @verse_start, @verse_end, @title, @content)
    `)
